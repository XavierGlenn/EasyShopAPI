...
public class AuthenticationController {
    ...
    public ResponseEntity<LoginResponseDto> login(@Valid @RequestBody LoginDto loginDto) {
        ...
        try {
            ...
            httpHeaders.add(JWTFilter.AUTHORIZATION_HEADER, "Bearer Token: " + jwt);
            return new ResponseEntity<>(new LoginResponseDto(jwt, user), httpHeaders, HttpStatus.OK); }
        catch(Exception ex) {
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Whoops. Try again?"); }
    }
    ...
    public ResponseEntity<User> register(@Valid @RequestBody RegisterUserDto newUser) {
        ...
        try {
            ...
            if (exists)
            { throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Name already registered. Choose another."); }
